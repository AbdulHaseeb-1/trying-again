...
export function AssetIcon({ symbol, size = 32 }: { symbol: string; size?: number }) {
  ...
  return (
    <Svg accessibilityRole="image" accessibilityLabel={`${symbol} icon`} width={size} height={size} viewBox="0 0 40 40">
      ...
      {key === 'GOLD' ? <><Polygon points="8,13 30,13 36,28 3,28" fill="#DCA72C" /><Polygon points="8,13 16,7 29,7 30,13" fill="#FFD76A" /><Path d="M11 18h17" stroke="#FFF1A8" strokeWidth="2" strokeLinecap="round" /></> : null}
